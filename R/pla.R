...
#' data matrix. The corresponding sparse loadings are calculated either using \code{PMD} from the \code{PMA} package
#' or using \code{spca} from the \code{elasticnet} package.
#'
#' @param x a numeric matrix or data frame which provides the data for the
#' principal loading analysis.
#' @param method chooses the methods to calculate the sparse loadings. \code{PMD} uses the
#' method from REF(WittenEtAl2009) and \code{SPCA} uses the method from REF(Zou2006).
#' @param para when \code{method = "PMD"}: an integer giving the bound for the L1 regularization.
#' when \code{method = "SPCA"}: a vector containing the penalization parameter for each variable.
#' @param cor a logical value indicating whether the calculation should use the
#' correlation or the covariance matrix.
...
spla <- function(x,
  ...
  if (method == "PMD" && length(para) != 1)
    stop("Enter a single sparseness parameter when method = PMD") # 'PMD' oder so möglich?
  if (method == "SPCA" && length(para) != K)
    stop("Enter a penalization parameter for each loading when method = PCA") # 'PCA' oder so möglich?
