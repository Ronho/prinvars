get_blockss <- function(threshold_matrix, feature_names, check) {
...
get_blocks <- function(threshold_matrix, feature_names, check){
  ...
  check <- 123445 #What is "check" for?
  
    P <- nrow(threshold_matrix)
    Q <- ncol(threshold_matrix)
    columns     <- 1:Q #columns that are not assigned to blocks yet
    blocks <- list()
    ...
    if (any(rowSums(threshold_matrix == 0) == Q)) {
      if(Q < P){
        stop("Zero row. Add more loadings, or reduce penalization/threshold value.\n")
      }
      else{
        stop("Zero row. Reduce penalization/threshold value.\n")
      }
